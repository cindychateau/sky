...
$baseUrl = $_SERVER["SERVER_NAME"]."/sky/";
//$baseUrl = $_SERVER["SERVER_NAME"]."/";
	...
	class Core extends Database {
				...
				switch ($type) {
					...
					case 'baja':
						if ($permissions[$module][2] == 1) {
							...
							$boton = '<a class="'.$classes.' tip" data-toggle="tooltip" title="'.$title.'" data-original-title="'.$title.'" href="'.$link.'" data-id="'.$data_id.'" '.$extras.' ><button type="button" class="btn btn-link btn-danger btn-lg"><i class="fa '.$icon.'"></i></button></a>';
						}
						break;
					...
					case 'cambios':
						if ($permissions[$module][3] == 1) {
							...
							$boton = '<a class="'.$classes.' tip" data-toggle="tooltip" title="'.$title.'" data-original-title="'.$title.'" href="'.$link.'" data-id="'.$data_id.'" '.$extras.' ><button type="button" class="btn btn-link btn-primary btn-lg"><i class="fa '.$icon.'"></i></button></a>';
						}
						break;
		...
		public function hasPermission($carpeta, $cli_id, $type) {
			if (!isset($_SESSION)) {
				@session_start();
			}
			...
			//Se prepara la consulta para ejecución
			$consulta = $this->_conexion->prepare('SELECT permisos
												   FROM permisos_carpetas
												   WHERE siu_id = ?
												   AND car_id = ?
												   AND cli_id = ?');
			...
			//Se ejecuta la consulta
			try {
				$consulta->execute(array($user, $carpeta, $cli_id));
				$puntero = $consulta->fetch(PDO::FETCH_ASSOC);

				if($consulta->rowCount()) {
					$permisos = str_split($puntero['permisos']);
					switch (($type)) {
						case 1:
							$perm = ($permisos[0] == 1 ? true : false);
							break;
						case 2:
							$perm = ($permisos[1] == 1 ? true : false);
							break;	
						case 3:
							$perm = ($permisos[2] == 1 ? true : false);
							break;	
						case 4:
							$perm = ($permisos[3] == 1 ? true : false);
							break;	
					}
				}
