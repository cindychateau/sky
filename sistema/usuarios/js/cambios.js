...
function getRecord() {
	...
	$.ajax({
		...
		success: function(result){
			if(!result.error){
				$("#nombre").val(result.SIU_NOMBRE);
				$("#email").val(result.SIU_EMAIL);
				$("#estatus").val(result.SIU_ACTIVO);
				getClients(result.CLI_ID)
				getTiposUsuarios(result.SUP_ID);
			}
			else {
				window.location = "index.php";
			}
...
function getTiposUsuarios(tipo) {
	params = {};
	params.id = tipo;
	$.ajax({
		type: 'POST',
